...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        lifecycleScope.launchWhenCreated {
            Log.i("Testando", "passou aqui 1")
            val listResult: ListResult = firebaseStorage.reference.listAll().await()
            listResult.items.forEach {
                Log.i("Testando", "item name -> ${it.name} ")
                Log.i("Testando", "item url -> ${it.downloadUrl.await()} ")
            }
        }
